...
description: This page aims to explain the meaning of project processing status vis-a-vis submission flow.
...
8. Upon validation, the IPD staff performs the [review](/projects/review). The objective of the review is to determine whether the project qualifies in the Public Investment Program.
9. The `Reviewer` will then submit the review to the PIP Lead who will review the `review` output. If the lead finds the review in order, he/she will accept it.
10. The `IPD` then presents the validated and reviewed projects to the DA management where `approval` or `disapproval` is obtained.
11. The `IPD` staff then proceeds with the encoding of the project in the NEDA PIPOL System.

![Submission Flow](/images/submission_flow.png)

As projects are processed in the system, their status gets updated.
